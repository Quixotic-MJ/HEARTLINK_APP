...
from app.utils.security import get_current_admin_user, create_access_token
...
class TestActivityLogsApi(unittest.TestCase):
    ...
    def test_meal_logging_creates_zero_admin_activity(self):
        ...
        res = self.client.post("/api/meals/usr-patient-101", json=payload, headers=headers)
        self.assertEqual(res.status_code, 200)
        self.assertEqual(len(mock_db.admin_activity), initial_count)
    ...
    def test_exercise_logging_creates_zero_admin_activity(self):
        ...
        res = self.client.post("/api/exercises/logs/usr-patient-101", json=payload, headers=headers)
        self.assertEqual(res.status_code, 200)
        self.assertEqual(len(mock_db.admin_activity), initial_count)
