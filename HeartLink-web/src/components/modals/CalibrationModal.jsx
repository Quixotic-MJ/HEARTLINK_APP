...
const CalibrationModal = ({ isOpen, onClose, activeLog, onArchive }) => {
  ...
  const getStatusBadge = (status) => {
    switch (status) {
      ...
      case "Archived":
        return (
          <span className="inline-flex items-center gap-1.5 bg-[#F8FAFC] text-[#64748B] border border-[#E2E8F0] px-2.5 py-0.5 rounded-full text-[9.5px] font-semibold uppercase tracking-wider">
            <Archive size={12} /> ARCHIVED
          </span>
        );
      default:
        return (
          <span className="inline-flex items-center gap-1.5 bg-[#EAF5FC] text-[#2E9AE8] border border-[#CDE1F4] px-2.5 py-0.5 rounded-full text-[9.5px] font-semibold uppercase tracking-wider">
      ...
      <div className="relative w-full max-w-lg bg-[#FFFFFF] max-h-full rounded-2xl shadow-2xl border border-[#E2E8F0] flex flex-col animate-in fade-in zoom-in-95 duration-200 overflow-hidden text-[#0F172A]">
        ...
        <div className="flex items-center justify-between px-6 py-4.5 border-b border-[#E2E8F0] bg-[#FFFFFF] z-10">
          <div className="flex items-center gap-3">
            ...
            <div>
              <h3 
                className="text-[17px] font-medium text-[#0F172A] tracking-tight"
        ...
        <div className="flex-1 overflow-y-auto px-6 py-5 [scrollbar-width:none] [-ms-overflow-style:none] [&::-webkit-scrollbar]:hidden space-y-5">
          ...
          <div className="flex flex-col gap-3.5 bg-[#F8FAFC]/60 p-4 rounded-[10px] border border-[#E2E8F0]">
            <div className="flex items-center justify-between">
              <div>
                ...
                <div className="flex items-baseline gap-1.5">
                  ...
                  <span className="text-[11px] text-[#64748B] font-semibold uppercase">({activeLog.ml_predicted_tier || "Stable"})</span>
                </div>
              </div>
            ...
            <div className="grid grid-cols-2 gap-2 pt-2.5 border-t border-[#E2E8F0] text-[12px]">
              <div>
                <span className="text-[#89899C] uppercase font-semibold text-[10px]">Absolute Error:</span>
                <span className="ml-1.5 text-[#0F172A] font-bold">{activeLog.absolute_error != null ? `${activeLog.absolute_error} pts` : "N/A"}</span>
              </div>
              <div className="text-right">
            ...
            {activeLog.model_metadata && (
              <div className="pt-2 border-t border-[#E2E8F0] text-[11px] text-[#94A3B8] font-mono flex flex-col gap-0.5">
                <div>Model: <span className="text-[#0F172A] font-medium">{activeLog.model_metadata.model_identifier}</span></div>
                {activeLog.model_metadata.model_hash && <div className="truncate">Hash: <span className="text-[#0F172A] font-medium">{activeLog.model_metadata.model_hash.substring(0, 16)}…</span></div>}
              </div>
            )}
          </div>
          ...
          {/* Reviewer Details */}
          <div>
            <h4 className="text-[11px] font-semibold text-[#94A3B8] uppercase tracking-wider border-b border-[#E2E8F0] pb-1.5 mb-2.5">
              Reviewer info
            </h4>
            <div className="flex items-center gap-2.5 bg-[#FFFFFF] border border-[#E2E8F0] p-3 rounded-[8px]">
              <div className="w-7 h-7 rounded-full bg-[#F8FAFC] flex items-center justify-center text-[#64748B] shrink-0">
                <UserCircle size={16} />
              </div>
          ...
          <div>
            ...
            <button
              ...
              className="w-full text-left flex items-center justify-between p-3 rounded-[8px] border border-[#E2E8F0] bg-[#FFFFFF] hover:border-[#2E9AE8] hover:bg-[#F8FAFC]/40 transition-colors group cursor-pointer"
            >
              <div className="flex items-center gap-2.5">
                <div className="w-7 h-7 rounded-[6px] bg-[#EAF5FC] flex items-center justify-center text-[#2E9AE8]">
                  <FileText size={14} />
                </div>
                <div>
              ...
              <ExternalLink
                size={14}
                className="text-[#2E9AE8] opacity-70 group-hover:opacity-100 transition-opacity"
              />
            </button>
          </div>

          {/* Structured Calibration Metrics */}
          <div className="space-y-3.5 pt-2 border-t border-[#E2E8F0] text-[12px]">
            <div>
              <span className="text-[11px] font-semibold text-[#94A3B8] uppercase tracking-wider block mb-1">Reviewer confidence</span>
            ...
            <div>
              <span className="text-[11px] font-semibold text-[#94A3B8] uppercase tracking-wider block mb-1">Adjustment reasons</span>
              <div className="flex flex-wrap gap-1.5">
                {activeLog.adjustment_reasons && activeLog.adjustment_reasons.length > 0 ? (
                  activeLog.adjustment_reasons.map((code) => {
                    ...
                    return (
                      <span key={code} className="text-[11px] font-medium text-[#0F172A] bg-[#F8FAFC] border border-[#E2E8F0] px-2 py-0.5 rounded-[5px]">
                        {labelMap[code] || code}
                      </span>
                    );
                  })
            ...
            <div className="grid grid-cols-2 gap-2.5">
              <div className="bg-[#FFFFFF] p-2.5 rounded-[8px] border border-[#E2E8F0]">
                <span className="text-[10px] font-semibold text-[#94A3B8] uppercase tracking-wider block mb-0.5">Exercise Suitability</span>
                ...
                {activeLog.exercise_feedback?.notes && (
                  <p className="text-[10.5px] text-[#64748B] italic mt-0.5 leading-snug">"{activeLog.exercise_feedback.notes}"</p>
                )}
              </div>
              <div className="bg-[#FFFFFF] p-2.5 rounded-[8px] border border-[#E2E8F0]">
                <span className="text-[10px] font-semibold text-[#94A3B8] uppercase tracking-wider block mb-0.5">Recipe Suitability</span>
                <span className="text-[12px] font-bold text-[#0F172A] capitalize">
                ...
                {activeLog.recipe_feedback?.notes && (
                  <p className="text-[10.5px] text-[#64748B] italic mt-0.5 leading-snug">"{activeLog.recipe_feedback.notes}"</p>
                )}
              </div>
            </div>
          </div>
          ...
          <div className="space-y-3 pt-2 border-t border-[#E2E8F0]">
            <div>
              <h4 className="text-[11px] font-semibold text-[#94A3B8] uppercase tracking-wider border-b border-[#E2E8F0] pb-1.5 mb-2">
                Risk interpretation notes
              </h4>
              <div className="bg-[#F8FAFC]/60 border border-[#E2E8F0] p-3 rounded-[8px]">
                <p className="text-[12px] text-[#0F172A] leading-relaxed whitespace-pre-wrap italic font-medium">
        ...
        <div className="px-6 py-3.5 border-t border-[#E2E8F0] bg-[#FFFFFF] flex justify-between items-center shrink-0">
          <button 
            ...
            className="flex items-center gap-1.5 text-[12px] font-semibold text-[#64748B] hover:text-[#A93226] transition-colors px-3 py-1.5 rounded-[6px] hover:bg-[#F7E4E1] disabled:opacity-40 disabled:cursor-not-allowed cursor-pointer"
          >
