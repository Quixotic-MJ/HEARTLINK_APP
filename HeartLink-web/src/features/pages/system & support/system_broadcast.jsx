    ...
    <AdminLayout>
      <div className={UI.page.container} style={{ fontFamily: FONTS.sans }}>
        ...
        <div className="bg-[#FFFFFF] rounded-[10px] border border-[#E2E8F0] flex flex-col overflow-hidden shadow-2xs">
          <div className="p-4 border-b border-[#E2E8F0] bg-[#FFFFFF] flex gap-3">
            ...
            <table className="w-full text-left border-collapse min-w-[700px]">
              <thead>
                <tr className="border-b border-[#E2E8F0] bg-[#F8FAFC]/40">
                  <th className="py-3 px-4 sm:px-5 text-[10.5px] font-semibold text-[#94A3B8] uppercase tracking-[0.1em]">Date Published</th>
                  <th className="py-3 px-4 sm:px-5 text-[10.5px] font-semibold text-[#94A3B8] uppercase tracking-[0.1em]">Publisher</th>
                  <th className="py-3 px-4 sm:px-5 text-[10.5px] font-semibold text-[#94A3B8] uppercase tracking-[0.1em] w-1/2">Announcement Details</th>
                  <th className="py-3 px-4 sm:px-5 text-[10.5px] font-semibold text-[#94A3B8] uppercase tracking-[0.1em] text-right">Action</th>
                </tr>
              </thead>
              <tbody className="divide-y divide-[#E2E8F0]">
                {isLoading ? (
                  <tr>
                    <td colSpan="4" className="py-12 text-center">
                      <div className="flex justify-center items-center gap-2 text-[#64748B]">
                        <Loader2 size={16} className="animate-spin text-[#2E9AE8]" />
                  ...
                  <tr>
                    <td colSpan="4" className="py-12 text-center text-[#64748B] text-[13px] font-medium">
                  ...
                  filteredBroadcasts.map((b) => (
                    ...
                    >
                      <td className="py-3.5 px-4 sm:px-5 align-middle">
                        <span className="text-[#64748B] font-mono font-medium text-[12px] flex items-center gap-1">
                          <Clock size={11} /> {b.date}
                        </span>
                      </td>
                      <td className="py-3.5 px-4 sm:px-5 align-middle">
                        <span className="text-[#0F172A] font-semibold text-[12.5px]">
                      ...
                      <td className="py-3.5 px-4 sm:px-5 align-middle">
                        <div className="space-y-0.5 max-w-[440px]">
                          <div className="flex items-center gap-2 flex-wrap">
                            <span className="text-[#0F172A] text-[13px] font-bold truncate group-hover:text-[#2E9AE8] transition-colors">
                              {b.title || "Announcement"}
                            </span>
                            {getCategoryBadge(b.type)}
                          </div>
                          <p className="text-[#64748B] text-[12px] font-medium truncate">{b.message}</p>
                        </div>
                      </td>
                      <td className="py-3.5 px-4 sm:px-5 align-middle text-right" onClick={(e) => e.stopPropagation()}>
                        <button 
                          ...
                          className="text-[12px] font-semibold px-2.5 py-1 rounded-[6px] border border-[#E2E8F0] bg-[#F8FAFC] text-[#0F172A] hover:bg-[#E2E8F0] transition-colors shadow-2xs inline-flex items-center gap-1 whitespace-nowrap cursor-pointer"
                        >
      ...
      {isConfirmModalOpen && (
        <div className="fixed inset-0 z-[60] flex items-center justify-center p-4" style={{ fontFamily: "'Inter', sans-serif" }}>
          ...
          />
          <div className="bg-[#FFFFFF] p-6 rounded-2xl shadow-2xl max-w-md w-full relative animate-in fade-in zoom-in-95 duration-200 border border-[#E2E8F0] text-[#0F172A] flex flex-col max-h-[90vh] overflow-hidden">
            <div className="flex items-center gap-3 mb-3.5">
              ...
              <div>
                ...
                <p className="text-[11px] text-[#64748B] font-medium">Review details before publishing to all users</p>
              </div>
            </div>

            <div className="flex-1 overflow-y-auto space-y-3 [scrollbar-width:none] [-ms-overflow-style:none] [&::-webkit-scrollbar]:hidden py-1">
              {/* Title & Category */}
              <div className="bg-[#F8FAFC]/60 p-3.5 rounded-[8px] border border-[#E2E8F0] space-y-1.5">
                <div className="flex items-center justify-between gap-2">
                  <span className="text-[10px] font-semibold text-[#94A3B8] uppercase tracking-wider">Title</span>
                  {getCategoryBadge(pendingBroadcast?.type)}
                </div>
                <p className="text-[13px] font-bold text-[#0F172A]">{pendingBroadcast?.title || "Untitled Announcement"}</p>
              </div>

              {/* Audience */}
              <div className="flex items-center justify-between px-3.5 py-2.5 bg-[#F8FAFC]/60 rounded-[8px] border border-[#E2E8F0] text-[12px]">
                <span className="text-[10px] font-semibold text-[#94A3B8] uppercase tracking-wider">Audience</span>
                <span className="font-bold text-[#0F172A]">{pendingBroadcast?.targetAudience || "All Registered Accounts"}</span>
              </div>

              {/* Message Preview */}
              <div className="space-y-1">
                <span className="text-[10px] font-semibold text-[#94A3B8] uppercase tracking-wider block">Message Preview</span>
                <div className="bg-[#F8FAFC]/40 rounded-[8px] border border-[#E2E8F0] p-3 max-h-36 overflow-y-auto">
                  <p className="text-[12.5px] text-[#0F172A] leading-relaxed whitespace-pre-wrap select-text font-medium">
                    {pendingBroadcast?.message}
                  </p>
                </div>
              </div>

              <p className="text-[11.5px] text-[#64748B] leading-relaxed pt-1">
                This message will be published to <strong className="text-[#0F172A]">{pendingBroadcast?.targetAudience || "all users"}</strong> immediately. This action cannot be undone.
              </p>
            </div>

            <div className="flex gap-2.5 pt-3.5 border-t border-[#E2E8F0] mt-3">
              <button 
                disabled={isSubmitting} 
                onClick={() => setIsConfirmModalOpen(false)} 
                className="flex-1 px-4 py-2 text-[12px] font-semibold text-[#0F172A] hover:bg-[#E2E8F0] bg-[#F8FAFC] rounded-[8px] transition-colors border border-[#E2E8F0] shadow-2xs disabled:opacity-50 cursor-pointer"
              >
                Cancel
              </button>
              <button 
                ...
                className="flex-1 px-4 py-2 text-[12px] font-semibold text-white bg-[#2E9AE8] hover:bg-[#1C7AC8] rounded-[8px] shadow-2xs transition-colors flex items-center justify-center gap-1.5 disabled:opacity-50 cursor-pointer"
              >
      ...
      {toast && (
        <div className={`fixed bottom-6 right-6 z-[70] px-4 py-3 rounded-[10px] shadow-2xl border flex items-center gap-2.5 animate-in slide-in-from-bottom-5 duration-300 bg-[#FFFFFF] ${toast.type === 'error' ? 'border-[#F0C4B8] text-[#A93226]' : 'border-[#C5DFD8] text-[#1B6E63]'}`}>
          ...
          <span className="text-[12.5px] font-semibold text-[#0F172A]">{toast.message}</span>
        </div>
      )}
    </AdminLayout>
  );
